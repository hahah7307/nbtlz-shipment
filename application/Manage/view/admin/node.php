...
<div class="layui-body" id="LAY_app_body">
    <div class="right">
		...
		<div class="layui-form">
			<a class="layui-btn" href="{:url('node_add')}">添加</a>
						...
						<tr>
							...
							<td class="tc">
								{if condition="$v.level neq '3'"}
									<a href="{:url('node_add', ['id' => $v.id])}" class="layui-btn layui-btn-sm">添加</a>
								{/if}
								{if condition="$v.level neq '1'"}
									<a href="{:url('node_edit', ['id' => $v.id])}" class="layui-btn layui-btn-normal layui-btn-sm">编辑</a>
...
layui.use(['form', 'jquery'], function(){
	...
	form.on('submit(Sort)', function(data){
		...
		button.text('请稍候...');
		$.ajax({
			type:'POST',url:"{:url('node_sort')}",data:data.field,dataType:'json',
	...
	form.on('switch(formLock)', function(data){
		$('button').attr('disabled',true);
		$.ajax({
			type:'POST',url:"{:url('node_status')}",data:{id:data.value,type:'look'},dataType:'json',
			success:function(data){
				if(data.code == 0){
					layer.alert(data.msg,{icon:2,closeBtn:0,title:false,btnAlign:'c'},function(){
						location.reload();
	...
	form.on('submit(Detele)', function(data){
		...
		layer.confirm('确定删除吗？',{icon:3,closeBtn:0,title:false,btnAlign:'c'},function(){
			...
			button.text('请稍候...');
			$.ajax({
				type:'POST',url:"{:url('node_delete')}",data:{id:id},dataType:'json',
				success:function(data){
					if(data.code == 1){
						layer.alert(data.msg,{icon:1,closeBtn:0,title:false,btnAlign:'c'},function(){
							location.reload();
