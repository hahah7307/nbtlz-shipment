		...
		<div class="layui-form">
				...
				<div class="layui-input-block w300">
					<select name="category_id" lay-verify="required">
						{foreach name="category" item="v"}
							<option value="{$v.id}" {if condition="$v.level lt 2"}disabled{/if}>{$v.category_name}({$v.code})</option>
						{/foreach}
					</select>
				</div>
			</div>
            <div class="layui-form-item">
                ...
                <div class="layui-input-block w300">
                    <select name="attribute_id" lay-verify="required">
                        ...
                        <option value="{$va.id}" {if condition="$va.level lt 2"}disabled{/if}>{$va.attribute_name}({$va.code})</option>
                        {/foreach}
                    </select>
                </div>
            </div>
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline w300">
                    <input type="text" class="layui-input" name="index" placeholder="请填写货号序号">
                </div>
            </div>
			<div class="layui-form-item">
...
layui.use(['form', 'jquery'], function(){
	...
	form.on('submit(formCoding)', function(data){
		...
		button.text('请稍候...');
        axios.post("{:url('create', ['id' => $id])}", data.field)
